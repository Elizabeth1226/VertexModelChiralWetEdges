...
def V_ave_func(filepath, time):
    """
    Calculate velocity magnitude average function
    ...
    cbins = np.arange(0, perioY + 0.5 * perioY / np.sqrt(Nc), 0.5 * perioY / np.sqrt(Nc))
    Nb = len(cbins)
    ...
    vr_list = np.zeros(Nb)
    vphi_list = np.zeros(Nb)
    bin_count = np.zeros(Nb)

    for cid in range(Nc):
        vr = (cVelocities[cid, 0] * xpos_center[cid] + cVelocities[cid, 1] * ypos_center[cid]) / np.sqrt(xpos_center[cid]**2 + ypos_center[cid]**2)
        vphi = (-cVelocities[cid, 0] * ypos_center[cid] + cVelocities[cid, 1] * xpos_center[cid]) / np.sqrt(xpos_center[cid]**2 + ypos_center[cid]**2)
        vr_list[cCategories[cid]] += vr
        vphi_list[cCategories[cid]] += vphi
        bin_count[cCategories[cid]] += 1

    for bin_id in range(Nb):
        if bin_count[bin_id] > 0:
            vr_list[bin_id] /= bin_count[bin_id]
            vphi_list[bin_id] /= bin_count[bin_id]

    v_list = np.sqrt(vr_list**2 + vphi_list**2)
