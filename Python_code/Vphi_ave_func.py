...
def Vphi_ave_func(filepath, time):
    """
    Calculate azimuthal velocity average function
    ...
    cbins : numpy array
        Bin positions
    vphi_list : numpy array
        Azimuthal velocity list
    """
    ...
    for cid in range(Nc):
        vphi = (-cVelocities[cid, 0] * ypos_center[cid] + cVelocities[cid, 1] * xpos_center[cid]) / np.sqrt(ypos_center[cid]**2 + xpos_center[cid]**2)
